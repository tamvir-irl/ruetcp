...
const PostPage = () => {
  ...
  const [initTitle, setInitTitle] = useState(""); // Initial title state
  const [initContent, setInitContent] = useState(""); // Initial content state
      ...
      if (searchQuery.length > 0) {
        try {
          const response = await fetch(
            `${siteConfig.links.api}/searchUser?q=${searchQuery}`
          );
          if (response.ok) {
    ...
    const fetchBlog = async () => {
      ...
      try {
        ...
        if (response.ok) {
          ...
          setInitTitle(blogData.blog.title);
          setInitContent(blogData.blog.content);
           // This updates the blog state
        } else {
  ...
  const handleSubmit = () => {
    ...
    //
    fetch(`${siteConfig.links.api}/blogs/edit/${blog?._id}`, {
      ...
      <Card>
        <CardHeader className="flex flex-col gap-3">
          ...
          <ButtonGroup>
            ...
            <Dropdown>
              <DropdownTrigger>
                <Button isIconOnly variant="flat" aria-label="Mention user">
                  <AtSign size={18} />
                </Button>
              </DropdownTrigger>
              <DropdownMenu
                ...
                <DropdownItem key={"usernames"}>
                  <div className="w-full flex flex-col justify-center">
                    {filteredUsers.map((user) => (
                      <Button
                        className="mb-2 rounded-xl ml-4"
                        variant="flat"
                        color="primary"
                        key={user.username}
                        onPress={() => handleUserMention(user.username)}
                      >
                        @{user.username}
                      </Button>
                    ))}
        ...
        <CardFooter className="flex justify-between">
          <p className="text-small text-default-500">Edit the post</p>
          <Button color="primary" onPress={() => setIsModalOpen(true)} isDisabled={(initTitle == title) && (initContent == content)}>
            Edit
          </Button>
        </CardFooter>
      </Card>
